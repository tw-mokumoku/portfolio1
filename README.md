...
#### **ご参考**: より具体的なイメージをお持ちいただくために、競合サイトである [ディスボード（https://disboard.org/ja）](https://disboard.org/ja)をご覧いただくことをお勧めします。
...
![image](https://github.com/tw-mokumoku/portfolio1/blob/master/doc/draw.PNG)
...
さて、最後に、本プロジェクトの遂行にあたって発生した費用について、詳細にご説明させていただきます。

![image](https://github.com/tw-mokumoku/portfolio1/blob/master/doc/Cost%20Explorer202403-202501.PNG)

上記のCost Explorerの画像に示されている通り、プロジェクト全体の **合計コストは$452.02（約69,611円）** となりました。（※ 2025年4月14日現在の為替レート1ドル154円で換算）

プロジェクト期間全体で均すと、 **1か月あたりの平均コストは約$41.09（約6,328円）** でした。

詳細な内訳を見てみますと、主に開発および運用に注力していた2024年6月から11月にかけて、インフラストラクチャーの利用に伴う費用が顕著に増加しました。この期間が最もコストが高く、特に費用が嵩んだ月では **$103（約15,862円）** にも達しました。これは、プロジェクトの進捗に伴い、必要なリソースを一時的に多く利用したためと考えられます。

これらの開発および運用に利用していた各種サービスは、プロジェクトの終了に伴い、現在はすべて停止し、関連するリソースも削除済みとなっております。また、残念ながら当時の環境や設定に関するバックアップを取得していなかったため、現時点ではAWSの具体的なポートフォリオとして構成や設定の詳細を提示することができません。しかしながら、実際にAWSの様々なサービスを設計、構築、運用した経験を通じて、クラウドインフラストラクチャーに関する実践的な知識と理解を深く習得できたと、今改めて強く実感しております。この経験は、今後の開発活動においても非常に貴重な財産となると確信しております。
